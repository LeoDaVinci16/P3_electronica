...
\begin{abstract}
    Aquest informe documenta el disseny, implementació i validació d'un sistema d'emulació ciber-física per a la gestió de tercer nivell d'una micro-xarxa DC híbrida. El sistema utilitza dades reals d'irradiància de Barcelona (2023) per emular el comportament del bus elèctric, l'emmagatzematge i la gestió de càrregues en un entorn de temps virtual accelerat. Mitjançant la integració d'un ESP32 i una interfície SCADA en Python, es demostra la capacitat del sistema per garantir l'estabilitat del bus a 40V i la continuïtat de càrregues crítiques davant la variabilitat climàtica anual, utilitzant estratègies de \textit{load shedding} i filtratge de senyal digital.
\end{abstract}
...
\begin{itemize}
    ...
    \item Emulació del funcionament anual de la xarxa en temps virtual accelerat.
...
\begin{equation}
    I_{cond} = I_{solar} + I_{grid} - I_{cons} - I_{loss}
\end{equation}
\begin{equation}
    V_{bus_{k+1}} = V_{bus_k} + \frac{I_{cond}}{C} \cdot \Delta t
\end{equation}
...
\begin{itemize}
    ...
    \item \textbf{Emulació sobre Planta Física:} \texttt{main.py} s'executa sobre el maquinari real. Les dades ADC (Pin 34) són ara l'única veritat del sistema, eliminant la dependència del simulador per al càlcul de la tensió.
\end{itemize}
...
Si la generació solar és màxima i no hi ha consum, la tensió del bus pot pujar ràpidament. A 45V, el \textit{Brain} activa el \textbf{Safety Sink}: força el LED verd al 100\% per drenar energia. Si la tensió persisteix per sobre de 45V, el DAC solar es posa a 0 per tallar la producció.

\section{Anàlisi i Resultats}
El resultat principal d'aquest projecte és l'emulació exitosa d'un escenari energètic complex. El sistema no es limita a un control de laboratori, sinó que se sotmet a una \textbf{validació climàtica real}.

\subsection{Validació amb Dades Reals i Temps Virtual}
L'execució del programa \texttt{main.py} processa el fitxer \texttt{irradiancia.csv}, que conté dades horàries reals de Barcelona durant l'any 2023. Atès que el bucle de control s'executa cada 50ms però avança l'índex de dades solars en cada iteració, el sistema opera en un \textbf{temps virtual accelerat}. Això permet observar en pocs minuts com es comportaria la micro-xarxa davant de variacions estacionals, dies ennuvolats o pics de radiació estivals.

Els gràfics de \textit{pyqtgraph} mostren una estabilitat del bus amb un error inferior a l'1\% en mode hardware real. La resposta del \textit{load shedding} és gairebé instantània (50ms), el que garanteix la supervivència de la càrrega roja fins i tot en caigudes sobtades de producció solar emulades.

\section{Conclusions}
Aquest projecte ha permès dissenyar un banc de proves funcional per a l'emulació de micro-xarxes. S'ha demostrat que la combinació de dades històriques reals i control físic en temps real és una eina poderosa per predir l'eficàcia de polítiques de \textit{load shedding}. La transició cap a un model d'emulació anual posa de manifest que la integritat del sistema depèn no només de l'electrònica de potència, sinó de la intel·ligència del programari per gestionar el balanç energètic en escenaris climàtics canviants.
